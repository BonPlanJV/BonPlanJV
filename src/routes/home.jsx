  ...
  useEffect(() => {
    ...
    const fetchGamesWithDetails = async () => {
      const games = getFavorites
        ? await getFavoriteGames(userID)
        : await readData("games");
      const gamesWithDetails = await formatGames(games);

      getFavorites
        ? setFavorite(gamesWithDetails)
          ...
          <div className="h-full w-full bg-neutral-800 p-5 flex justify-center">
            <div className="w-[90%] h-full space-y-10">
              ...
              <div className="flex flex-wrap space-y-5">
                {getFavorites ? (
                  favorites.map((game) => <Game key={game.key} game={game} />)
                ) : gamesArray.length > 0 ? (
                  gamesArray.map((game) => <Game key={game.key} game={game} />)
                ) : (
